...
public class ReflexionUtil {
    ...
    //**********************************************
    public static Object launchGetter(Object obj, int getterNumber) throws Exception {
        List<Method> methods = getGetterList(obj.getClass());
        if (getterNumber < methods.size()) {
            return launchGetter(obj, methods.get(getterNumber));
        }

        return null;
    }

//    public static Method launchGetterByName(Object obj, String name) throws Exception {
//        Class[] params = new Class[1];
//        params[0] = Class.forName("java.lang.String");
//        return obj.getClass().getMethod(name, params);
//    }
    public static Object launchGetterByName(Object obj, String name) throws Exception {
        List<Method> getters = getGetterList(obj.getClass());
        for (Method getter : getters) {
            //System.out.println("haaa les getteers ==> " + getter.getName());
        }
        for (int i = 0; i < getters.size(); i++) {
            Method method = getters.get(i);
            if (method.getName().toLowerCase().equals(("get" + name).toLowerCase())) {
                System.out.println("hani l9ite le getter dont le nom est " + name);
                return launchGetter(obj, method);
            }
    ...
    public static Object launchGetterByParamName(Object obj, String paramName) throws Exception {
        List<Method> getters = getGetterList(obj.getClass());
        for (int i = 0; i < getters.size(); i++) {
            Method method = getters.get(i);
    ...
    public static Object launchGetter(Object objet, Method methodGetter) throws Exception {
        String type = methodGetter.getReturnType().getSimpleName();

        if (!type.equals("List") && !type.equals("ArrayList")) {
            Object obj = methodGetter.invoke(objet, null);
            //System.out.println("LunchGetter Object==> "+ obj);
            return obj;
        }
        return null;
    }

    public static Object launchGetterWithParams(Object objet, Method methodGetter, String param) throws Exception {
    ...
    public static Object makeInstance(Class myClass, List<Method> setters, List<Object> values) {
        ...
        try {
            ...
            for (int i = 0; i < values.size(); i++) {
                ...
                launchSetter(myClass, setters.get(i), value);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static List<String> lunchGetters(Object obj) throws Exception {
        ...
        for (int i = 0; i < methods.size(); i++) {
            Method method = methods.get(i);
            Object object = launchGetter(obj, method);
            if (object != null) {
